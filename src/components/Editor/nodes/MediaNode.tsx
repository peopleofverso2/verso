...
import { MediaFile, MediaNodeProps } from '../../../types/media';
...
const MediaNode: React.FC<MediaNodeProps> = ({ id, data, selected }) => {
  ...
  // Fonction de sélection de média
  const handleMediaSelect = useCallback(async (selectedMedia: MediaFile[]) => {
    ...
    try {
      ...
      setError('Erreur lors de la sélection du média');
    }
  }, [id, data]);

  // Fonction de sélection audio
  const handleAudioSelect = useCallback(async (selectedMedia: MediaFile[]) => {
    if (!selectedMedia || selectedMedia.length === 0) return;
    
    const audioFile = selectedMedia[0];
    ...
    try {
      ...
      setError('Erreur lors de la sélection de l\'audio');
    }
  }, [id, data]);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <DialogContent>
          <MediaLibrary
            ...
            multiSelect={false}
            initialSelectedMedia={data.mediaId ? [{ metadata: { id: data.mediaId } } as MediaFile] : []}
          />
        </DialogContent>
      </Dialog>
      ...
      <Dialog
        ...
        fullWidth
      >
        <DialogTitle>Sélectionner un audio</DialogTitle>
        <DialogContent>
          <MediaLibrary
            onSelect={handleAudioSelect}
            multiSelect={false}
            initialSelectedMedia={data.audioId ? [{ metadata: { id: data.audioId } } as MediaFile] : []}
          />
        </DialogContent>
      </Dialog>
