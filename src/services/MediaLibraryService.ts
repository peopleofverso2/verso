...
export class MediaLibraryService {
  ...
  async updateMetadata(id: string, metadata: Partial<MediaMetadata>): Promise<MediaFile> {
    ...
    try {
      ...
      // Mettre à jour les métadonnées via l'adaptateur de stockage
      const updatedMedia = await this.storageAdapter.updateMetadata(id, metadata);
      console.log('Metadata updated successfully:', updatedMedia);

      // Mettre à jour le cache d'URL si nécessaire
